...
const server = createServer((req, res) => {
  ...
  try {
    ...
    // Route handling
      ...
      // Simulate changing intensity
      if (Math.random() > 0.7) { // 30% chance of intensity change
        currentIntensity = Math.max(1, Math.min(10, currentIntensity + randomInt(-1, 2)));
        
        // Broadcast intensity change via SSE
        broadcastToClients('seismic.update', {
          intensity: currentIntensity,
          velocity: 0.15 + Math.random() * 0.1,
          acceleration: 0.92 + Math.random() * 0.2,
          timestamp: new Date().toISOString(),
      ...
      // Simulate new alerts occasionally (only intensity 5+)
      if (Math.random() > 0.9) { // 10% chance of new alert
        const alertIntensity = randomInt(5, 10); // Only 5-10 for alerts
        const newAlert = {
          id: Date.now().toString(),
          intensity: alertIntensity,
          acceleration: randomInt(8, 45) / 100, // Higher acceleration for higher intensities
          timestamp: new Date().toISOString(),
          device_id: 'device-001',
...
setInterval(() => {
  ...
  if (Math.random() > 0.8) {
    currentIntensity = Math.max(1, Math.min(10, currentIntensity + randomInt(-2, 3)));
  ...
  // Only generate alerts for intensity levels 5 and above (PEIS scale)
  if (Math.random() > 0.92) {
    const alertIntensity = randomInt(5, 10); // Only 5-10 for alerts
    const newAlert = {
      id: Date.now().toString(),
    ...
    console.log(`New alert added: intensity ${alertIntensity} (PEIS ${alertIntensity >= 7 ? 'WARNING' : 'NOTICE'})`);
    
    // Broadcast new alert via SSE
    broadcastToClients('seismic.alert', newAlert);
  }
}, 5000); // Update every 5 seconds (slower, more manageable)
